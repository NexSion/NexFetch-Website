...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Carry the pending device_key through the email-confirmation link
    // so /auth/callback can claim it the moment the session exists —
    // see that route for the other half of this.
    const redirect = new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/auth/callback`);
      ...
      <div className="mx-auto flex min-h-[70vh] max-w-md items-center px-6">
        <GlassCard className="w-full glow-border text-center">
          ...
          <p className="mt-3 text-sm text-white/60">
            ...
            activate your account{dkey ? " — it'll link this browser automatically" : ""}.
          </p>
        </GlassCard>
      </div>
    );
    ...
    <div className="mx-auto flex min-h-[70vh] max-w-md items-center px-6">
      <GlassCard className="w-full glow-border">
        ...
        <p className="mt-6 text-center text-sm text-white/50">
          ...
          <Link href={dkey ? `/login?dkey=${encodeURIComponent(dkey)}` : "/login"} className="text-blue-glow hover:underline">
            Log in
          </Link>
        </p>
      </GlassCard>
